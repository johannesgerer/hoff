...
traceMeta :: ToH Table b => b -> b
traceMeta x = Debug.Trace.trace (show $ meta x) x
{-# INLINABLE traceMeta #-}
...
class TableList l where
  toTableList :: l -> [(Maybe Text, Table)]
instance TableList [Table] where
  toTableList = fmap (Nothing,) 
instance TableList [TableH] where
  toTableList = toTableList . fmap unsafeH
instance ConvertText t Text => TableList (Map t Table) where
  toTableList = fmap (first $ Just . toS) . M.toList
instance ConvertText t Text => TableList (Map t TableH) where
  toTableList = toTableList . fmap unsafeH
instance ConvertText t Text => TableList (HM.HashMap t Table) where
  toTableList = fmap (first $ Just . toS) . HM.toList
instance ConvertText t Text => TableList (HM.HashMap t TableH) where
  toTableList = toTableList . fmap unsafeH

printMetas :: (MonadIO m, TableList a) => a -> m ()
printMetas = mapM_ (\(n,r) -> mapM print n >> print (meta r) >> print (count r) >> putStrLn @String "\n")
  . toTableList
...
printSummaries = mapM_ (\(n,r) -> mapM print n >> print (summary r) >> putStrLn @String "\n")
  . toTableList
