...
instance ValueShow Table where
  buildSingleLine t = "+" <> buildSingleLine (flipTable t) 
  showValueB = showbPrec 0

instance ValueShow TableRow where
  buildSingleLine t = "+" <> buildSingleLine (flipTable t) 
  showValueB = showbPrec 0


instance Show Table where show = showv
instance Show TableCol where show = showv
instance Show TableRow where show = showv
instance Show TableCell where show = showSingleton

instance TextShow TableRow where
  showbPrec x = showbPrec x . tableRowToTable

instance TextShow Table where
  showbPrec _ = unlinesB . V.toList . combineDictBuilder . getDictBuilder
...
showSingleton :: forall a c . (ConvertText LText c, AtomShow a) => a -> c
