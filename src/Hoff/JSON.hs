...
decodeVectorOfRecordsWith :: [Text] -> LByteString -> Either String Table
decodeVectorOfRecordsWith nulls = left snd . (eitherDecodeWith json' $ iparse $ parseVectorOfRecords nulls)

decodeKeyToRecordMapWith :: [Text] -> LByteString -> Either String (SingleKeyTable Text)
decodeKeyToRecordMapWith nulls = left snd . (eitherDecodeWith json' $ iparse $ parseKeyToRecordMap nulls)
...
instance FromJSON (ColumnToVectorMap Table) where
  parseJSON = withObject "Hoff.TypedTable via ColumnToVectorMap" $
...
instance HS.ToJsonAble (ColumnToVectorMap Table) where
  toJsonWithError = HS.toJsonWithError . flipTable . fromColumnToVectorMap

-- | not an efficiently implementation
instance HS.ToJsonAble (VectorOfRecords Table) where
  toJsonWithError = fmap (A.toEncoding &&& A.toJSON) . mapM HS.ensureToJson
    <=< runHEither . exec rowDict . fromVectorOfRecords

-- | use via `Hoff.Serialise.ensureToJson`
instance HS.EncodableContainer f => HS.ToJsonAble (TableDict' f) where
  toJsonWithError t = fmap (f . V.unzip)
    $ V.zipWithM (\k v -> ((k,) *** (fromText k,)) . snd <$> HS.encodeCol (k, v)) (key t) $ value t
