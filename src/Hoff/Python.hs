...
pythonEvalHoffs pBinary pp = fmap2 cborToTables . pythonEvalHoffCbor pBinary pp
...
pythonEvalHoff pBinary pp = fmap2 cborToTable
  . pythonEvalHoffCbor pBinary (pp & pSource_ %~ singleTableSourceMod)
{-# INLINABLE pythonEvalHoff #-}

-- | common helper function
pythonEvalHoffCbor :: (ToJSON j, HasCallStack) =>
  FilePath -> PythonProcess j -> Either EoTable [(Text,EoTable)] -> (Maybe ByteString, IO ByteString)
pythonEvalHoffCbor pBinary pp inputTables = (inputBs,) $ do
  modulePath <- Hoff.Serialise.pythonModule
  pythonEvalByteString pBinary $ pp
    { pInput = inputBs
    , pSource =T.unlines [importHoff modulePath, inputCode
                         ,appendColumnConversion pp, outputDfs]} 
...
appendColumnConversion :: PythonProcess input -> Text
appendColumnConversion PythonProcess{..} = case pColumnNames of
  OriginalColNames -> pSource
  CamelCaseColNames -> pSource <> camelCaseOutputDfColumns
