...
tableColD name = HQueryDyn $ NamedTableReader $ (Just name, ReaderT g)
  where g (GroupedTable{..}, _) = mapWrappedM (gtToGroups "tableColD") =<< getTableCol name gtTable
{-# INLINABLE tableColD #-}

getTableCol :: (HasCallStack, ToTable t) => Symbol -> t -> H TableCol
getTableCol name = chainToH $ \t ->
  mapHException (appendMsg $ toS $ "\n\n" <> showMetaWithDimensions t) $ flipTable t ! name
{-# INLINABLE getTableCol #-}
...
--
-- see also `fby` for a version with different grouping
--
-- see also `ae`
--
...
instance Zippable (NamedTableReader f) where
  zipWith_ = liftA2
  zipWith3_ = liftA3
  zipWith4_ = liftA4
  zipWith5_ = liftA5
