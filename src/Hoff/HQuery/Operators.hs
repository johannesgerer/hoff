...
rowDict :: Exp I TableRowDict
rowDict = noName $ ReaderT $ coerce . rowDicts . gtTable
...
melt :: (HasCallStack, ToTable t)
  ...
  -> t -> TableH
melt cols measureColName valueColName = chainToH @Table $ \t -> do
  neCols <- noteH (TableWithNoColumn "melt needs non-empty list of columns to melt") $ nonEmpty cols
  t2 <- delete cols t
  concatMatchingTables <=< forM neCols $ \cn -> getTableCol cn t
...
-- ^ rows that evaluate to (False, True)
partition wh = chainToH $ \t -> runWhere wh t >>= \w ->
  (,) <$> applyWhereResult t (invert w) <*> applyWhereResult t w
...
partitionMaybes3 performFromJust cols = chainToH $ \t -> bool (g t) (pure (H.take 0 t, t)) $ V.null cols
  where
    g = mapM (bool pure (update $ fromJustD . ad <$> toList cols) performFromJust)
      <=< partition (V.foldl1 (zipWith_ (&&)) $ isJust_ . ad <$> cols)
