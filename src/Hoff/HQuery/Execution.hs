...
-- | map to row
execRecAggBy :: forall g r t . (ToTable t, HasCallStack, TableRowT r, Wrappable g)
...
execRecAggByW :: forall g r t . (ToTable t, HasCallStack, TableRowT r, Wrappable g)
...
execMap :: forall v g t . (ToTable t, HasCallStack, Wrappable g, Wrappable v)
  => Exp I v -> Exp I g -> t -> VectorDictH g v
execMap va ke t = chain2 dict (exec ke t) $ exec va t
{-# INLINABLE execMap #-}

execKey :: forall v g t . (ToKeyedTable t, HasCallStack, Wrappable g, Wrappable v)
  => Exp I v -> Exp I g -> t -> VectorDictH g v
execKey va ke = chainToH @KeyedTable $ \kt -> chain2 dict (exec ke $ key kt) $ exec va $ value kt
{-# INLINABLE execKey #-}

execAggBy :: forall v g t . (ToTable t, HasCallStack, Wrappable g, Wrappable v)
...
execAggByW :: forall v g t . (ToTable t, HasCallStack, Wrappable g, Wrappable v)
...
sortT w by = chainToH $ sortByWithM (exec w) by
{-# INLINABLE sortT #-}
...
sortTD :: (HasCallStack, ToTable t) => ExpDyn I -> (forall f a . (Ord1 f, Ord a) => f a -> f a -> Ordering) -> t -> TableH
sortTD w by = chainToH $ \t -> withWrapped (\v -> sortByWith (const $ co v) by t)
