...
type TypedTable r = TypedTable' Vector r
...
fromTableCommon :: (HasCallStack, TableRow r) =>
  (forall a . (Typeable a, HasCallStack) => Maybe Symbol -> TableCol -> H (Vector a))
  -> Table -> TypedTableH r
fromTableCommon fromW t = fmap UnsafeTypedTable $ R.cmapM (Proxy @Typeable)
  (\(K name) -> fromW (Just $ toS name) =<< f name) $ R.reifyKnownFields Proxy
  where f = (flipTable t !) . toS
{-# INLINABLE fromTableCommon #-}
...
columns = chainToH $ fromTableCommon fromWrappedDyn
{-# INLINABLE columns #-}

columnsF :: forall r t . (HasCallStack,  ToTable t, TableRow r) => t -> H (TypedTable r)
columnsF = chainToH $ fromTableCommon fromWrappedDynF
{-# INLINABLE columnsF #-}
...
flipTypedTable :: (TableRow r, HasCallStack) => (forall a . a -> b a) -> TypedTable r -> Vector (Record b r)
