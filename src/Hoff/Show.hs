...
class ValueShow v where
  ...
  showValueB :: HasCallStack => v -> Builder
...
unlinesV :: V.Vector v Builder => v Builder -> Builder
unlinesV = unlinesB . V.toList
{-# INLINABLE unlinesV #-}

combineDictBuilder :: InDictBuilder -> VB.Vector Builder
combineDictBuilder (InDictBuilder w h ne b) = bool b (V.fromList [h, fromText $ TS.replicate w "-"] <> b) ne
...
{-# INLINABLE singleLineWithSuffix #-}
...
{-# INLINABLE buildBracketList #-}

showSingleLine :: (ConvertText LText c, ValueShow a) => a -> c
showSingleLine = builderToText . buildSingleLine
{-# INLINABLE showSingleLine #-}

builderToText :: ConvertText LText c => Builder -> c
builderToText = toS . toLazyText
{-# INLINABLE builderToText #-}

showv = builderToText . showValueB
{-# INLINE showv #-}
