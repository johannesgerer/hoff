...
-- import qualified Hoff.Examples2 as HE2
...
u_u2 =  #k <# [3,2,1,4::Int]  //
      tcF #d2 [Just 10, Just 1, Nothing, Just (2::Int)]
...
main = do
  hspec $ do
    ...
    describe "ijcFaster" $ do
      let cmp i (k,a,b,c) =  it ("should satisfy case " <> show i) $ flip shouldBe (meta c, c)
            $ let x = ijcFaster k a b  in (meta x, x)
          t = #s <# ("ifffim"::String) //
              #p <# [7,8,6,6,2,5::Int]
          s =  #s <# ("imm"::String) //
               #e <# ("nca"::String) //
    ...
    describe "ujk" $ do
      let cmp i (a,b,c) =  it ("should satisfy case " <> show i) $ flip shouldBe (a, b, meta c, c)
        ...
        ,let k = xkey [#k] in (k u_t, k u_sa, k u_u2)
    ...
    describe "grouping should preserve group row order within group" $ do
      let cmp i (x,c) =  it ("should satisfy case " <> show i) $ x `shouldBe` c
        in zipWithM_ cmp [(1::Int)..]
        [(show $ V.fromList [0:: Int,1,2,3,4],show $ unsafeH $ execAgg (ae rn) kxSTt)
        ,(show $ V.fromList [(True, V.fromList [0:: Int,1,2,3,4])]
         ,show $ unsafeH $ zipDict' $ execAggBy (ae rn) (co True) kxSTt)
        ]
    ...
    describe "summaryOld" $ do
      let asd = #a <# [None (), None (), None (), None ()] //
          ...
          cmp i (a,b) = it ("should satisfy case " <> show i) $ shouldBe (a, summaryOld a) (a, b)
        in zipWithM_ cmp [(1::Int)..]
           [(asd, suma)]
    ...
    describe "hQuery" $
      ...
      [descTD #c (aggBy [ai $ V.filter @Double (> 3.0) <$> #a] #c t1) `shouldBe`
       (#c <# ("ba" ::String) // #a <# [pure (7::Double), toVector [9,7]])
      ]
