...
ensureToJson :: ToJsonAble a => a -> Either Text (EncodeOnlyJ a)
ensureToJson a = UnsafeEncodeOnly a <$> toJsonWithError a

class ToJsonAble a where
  toJsonWithError :: a -> Either Text (A.Encoding, A.Value)
  default toJsonWithError :: A.ToJSON a => a -> Either Text (A.Encoding, A.Value)
...
encodeVectorM' encodeM = fmap (\es -> encodeListLen (fromIntegral $ VG.length es) <> VG.foldMap id es)
  . noErrors . vMapTellErrors encodeM
...
encodeTables :: HasCallStack => [(Text, Table)] -> Either Text C.Encoding
encodeTables ts = appendCallStack $ let ?callStack = freezeCallStack emptyCallStack in a
  where a :: HasCallStack => Either Text C.Encoding
        a = encode <$> mapErrors (\(n,t) -> fmap (n,) . left (("Table " <> n <> ":\n") <>) $ ensureEncodable t)
            ts
...
instance Encodable Table where 
  encodeWithError t = appendCallStack . left (\e -> e <> "\n\n" <> showMetaWithDimensions t)
    . encodeWithError . zipDict $ flipTable t


instance Encodable (Symbol, TableCol) where encodeWithError = fmap fst . encodeCol
instance Decodable (Symbol, TableCol) where decode' = decodeColPython
...
instance Decodable Table where
  decode' = runHorFail . table @Vector =<< decode'
...
encodeCol :: (EncodableContainer f, HasCallStack) => (Symbol, WrappedDyn f) -> Either Text (C.Encoding, (A.Encoding, A.Value))
encodeCol (colName, WrappedDyn tr@(App con1 tr2) wd) 
  ...
  where 
        ...
        b64 = decodeASCII . B64.encode
        bl64 = decodeASCIILazy . BL64.encode
...
decodeColPython :: HasCallStack => Decoder s (Symbol, TableCol)
decodeColPython = decodeListLenOf 3 >> do
  colName <- decode @Text
  fmap (colName,) $ decode @Text >>= \case
