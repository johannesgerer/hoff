...
appendMsg m = \case
  TableWithNoColumn             t -> TableWithNoColumn             $ t <> m
  CountMismatch                 t -> CountMismatch                 $ t <> m
  TypeMismatch                  t -> TypeMismatch                  $ t <> m
  TableDifferentColumnLenghts   t -> TableDifferentColumnLenghts   $ t <> m
  IncompatibleKeys              t -> IncompatibleKeys              $ t <> m
  IncompatibleTables            t -> IncompatibleTables            $ t <> m
  KeyNotFound                   t -> KeyNotFound                   $ t <> m
  UnexpectedNulls               t -> UnexpectedNulls               $ t <> m
  SqliteError                   t -> SqliteError                   $ t <> m
  SqlServerError                t -> SqlServerError                $ t <> m
...
runHEither = (_Left %~ shot) . hToEither
...
runHinIO :: H a -> IO a
runHinIO = runHWith (throwIO . Prelude.userError . show) pure
{-# INLINABLE runHinIO #-}
