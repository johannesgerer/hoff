...
-- basically raw bytestrings) as Utf8 (leniently decoded)
--
...
querySqlServer :: HasCallStack => AssumeUtf8Char -> Connection -> Query -> IO Table
...
instance ToSql          Day     where toSql     = toSql . toBaseDay
instance FromValue      Day     where fromValue = fmap2 fromBaseDay fromValue

instance ToSql          Date    where toSql     = toSql . toBaseDay . dateToDay
instance FromValue      Date    where fromValue = fmap2 dayToDate fromValue      

instance ToSql          TimeOfDay where toSql   = toSql . toBaseTimeOfDay
instance FromValue      TimeOfDay where fromValue= fmap2 fromBaseTimeOfDay fromValue

instance ToSql          Time    where toSql     = toSql . Datetime2 . toBaseLocalTime
instance FromValue      Time    where fromValue = fmap2 fromBaseLocalTime fromValue

-- | leniently decoded
--
-- useage:
--
-- `deriving (FromValue, ToqSql) via AsUtf8`
newtype AsUtf8 = AsUtf8 { fromAsUtf8 :: Text }

instance ToSql          AsUtf8    where toSql     = toSql . encodeUtf8 . fromAsUtf8
instance FromValue      AsUtf8    where fromValue = fmap2 (AsUtf8 . fromUtf8Lenient) fromValue
