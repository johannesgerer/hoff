...
import           Yahp hiding (get, group, groupBy, (&), TypeRep, take, drop, null, sortBy, reverse)
...
data Dict k v = UnsafeDict { unsafeKey          :: k
                           ...
                           , unsafeGroupingDict :: Dict k RawGrouping }
  -- deriving Functor <-- not safe, because length could be changed
  deriving (Generic, Generic1)

instance (NFData (KeyItem k), NFData k, NFData v) => NFData (Dict k v)
instance (NFData (KeyItem k), NFData k) => NFData1 (Dict k)

makeLensesWithSuffix ''Dict
...
instance Hashable k => Monoid (VectorDict k v) where
  mempty = UnsafeDict mempty mempty mempty mempty
...
instance HashableKey (Vector k) => Semigroup (VectorDict k v) where
  ...
  sconcat = toList >>> \ds -> dictNoLengthCheck (V.concat $ key <$> ds)
    $ V.concat $ value <$> ds
  {-# INLINABLE sconcat #-}
...
zipDict' :: VectorDictH k v -> VectorH (k, v)
zipDict' = fmap zipDict
{-# INLINABLE zipDict' #-}

zipDictWith' :: (k -> v -> a) -> VectorDictH k v -> VectorH a
zipDictWith' = fmap . zipDictWith
{-# INLINABLE zipDictWith' #-}

mapDictWithKeyM :: (Monad m, HashableDictComponent k) => (KeyItem k -> v -> m w) -> Dict k (Vector v) -> m (Dict k (Vector w))
mapDictWithKeyM f d = dictNoLengthCheck (key d) <$> V.zipWithM f (toUnsafeKeyVector $ key d) (value d)
{-# INLINABLE mapDictWithKeyM #-}
...
(!!) :: (HasCallStack, Typeable k, DictComponent v, Hashable k, Eq k, Show k)
  => Dict (Vector k) v -> Vector k -> H v
d !! k = flip unsafeBackpermute (value d) <$> safeMapAccess g k d
  where g hm = mapM $ \k -> noteH (KeyNotFound $ show (typeOf k) <> " " <> show k) $ hm HM.!? k
{-# INLINABLE (!!) #-}

(!!/) :: (HasCallStack, Typeable (KeyItem k), DictComponent v, HashableDictComponent k, Show (KeyItem k))
  => Dict k v -> k -> H v
d !!/ k = flip unsafeBackpermute (value d) <$> join (safeMapAccessH g k d)
  where g hm = mapM $ \k -> noteH (KeyNotFound $ show (typeOf k) <> " " <> show k) $ hm HM.!? k
{-# INLINABLE (!!/) #-}

safeMapAccessH :: (HasCallStack, DictComponent k)
  => (HM.HashMap (KeyItem k) IterIndex -> KeyVector k ->  a) -> k -> Dict k v -> H a
...
intersectAndDiffH s d = ffor (safeMapAccessH (V.mapMaybe . (HM.!?)) s d)
...
d !!?/ k = flip unsafeBackpermuteMaybe (value d) =<< safeMapAccessH (fmap . (HM.!?)) k d
{-# INLINABLE (!!?/) #-}
...
fillDictWithM fa fb fab a b = do
  ...
  (a { unsafeValue = a' } <>) <$> mapDictWithKeyM fb (unsafeRemoveByIndex b intersect)
...
groupBy_ g = dictNoLengthCheck gu . unsafeApplyGrouping idxs
...
getGroupingDict :: HashableDictComponent t => t -> Dict t RawGrouping
getGroupingDict = uncurry dictNoLengthCheck . second gSourceIndexes . getGroupsAndGrouping
...
instance {-# OVERLAPPING #-} AtomShow LText where
  buildAtomRaw = fromLazyText
...
instance AtomShow a => ValueShow (Vector a) where
  ...
  showValueB = if defaultIsSingleLine (Proxy @a) then buildSingleLine
    else unlinesV . buildSingleColumn
  {-# INLINABLE showValueB #-}
...
instance (DictComponentShow a, DictComponentShow b) => ValueShow (Dict a b) where
  ...
  showValueB d = unlinesV $ combineDictBuilder $ horizontalConcat "| " $ trans <$> (keys :| [vals])
    where (keys, vals) = (getDictBuilder $ key d, getDictBuilder $ value d)
          trans x = InDictBuilder 0 "" False $ combineDictBuilder
            $ x { hNonEmtpyHeader = on (||) hNonEmtpyHeader keys vals }
  {-# INLINABLE showValueB #-}

instance (DictComponentShow a, DictComponentShow b) => TextShow (Dict a b) where showb = showValueB

instance (DictComponentShow a, DictComponentShow b) => Show (Dict a b) where show = showv

-- * conversion to other maps

dictToMap :: Ord k => VectorDict k a -> Map k a
dictToMap = fromList . toList . zipDict
{-# INLINABLE dictToMap #-}

dictToHashMap :: Hashable k => VectorDict k v -> HM.HashMap k v
dictToHashMap = fromList . toList . zipDict
{-# INLINABLE dictToHashMap #-}
